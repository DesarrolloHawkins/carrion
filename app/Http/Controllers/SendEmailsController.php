...
class SendEmailsController  extends Controller
{
    ...
    public function obtenerClientesConExcesoDeReservas()
    {
        ...
        $reservasConOrder = Reservas::whereNotNull('order_id')->with('clientes', 'silla')->get();
        ...
        foreach ($reservasAgrupadasPorOrder as $order_id => $reservas) {
            ...
            if ($cantidadSillas > $limiteSillas) {
                $clientesConExcesoReservas[] = [
                    ...
                    'reservas' => $reservas->toArray(), // Convertir las reservas a array para facilitar la visualización
                    'cantidad_sillas' => $cantidadSillas,
                    'limite_permitido' => $limiteSillas
                ];
            }
        }
